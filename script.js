...
function rebuildTeamInputs(size) {
...
function setupTournamentUI() {
  rebuildTeamInputs(bracketSize);

  const customWrap  = document.getElementById('bracket-custom-wrap');
  const customInput = document.getElementById('bracket-size-custom-input');
  ...
  document.querySelectorAll('.bracket-size-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.size === 'custom') {
        ...
        customInput.focus();
        // Don't change bracketSize until the user actually enters a value
        return;
      }
  ...
  customInput.addEventListener('input', () => {
    ...
    if (!raw || raw < 2) return; // wait for a usable value before rebuilding
    const size = Math.min(64, Math.max(2, raw));
    bracketSize = size;
    rebuildTeamInputs(bracketSize);
  });
...
function generateBracket() {
  const customWrap  = document.getElementById('bracket-custom-wrap');
  const customInput = document.getElementById('bracket-size-custom-input');
  if (customWrap && !customWrap.hidden) {
    const raw = parseInt(customInput.value, 10);
    if (raw >= 2) {
      const size = Math.min(64, Math.max(2, raw));
      if (size !== bracketSize) {
        bracketSize = size;
        rebuildTeamInputs(bracketSize); // keep name inputs in sync with the final size
      }
    }
  }
